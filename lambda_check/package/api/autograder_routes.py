...
def list_artifacts_query(
    ...
    if len(queries) == 1 and queries[0].name == "*":
        for artifact_id, artifact in all_artifacts:
            # Apply type filter even for wildcard - treat empty list as no filter
            if not queries[0].types or artifact["type"] in queries[0].types:
        ...
        for query in queries:
            for artifact_id, artifact in all_artifacts:
                if artifact["name"] == query.name:
                    ...
                    if not query.types or artifact["type"] in query.types:
                        results.append(ArtifactMetadata(
                            name=artifact["name"],
                            id=artifact_id,
                            type=artifact["type"]
    ...
    # For wildcard queries, return all results (no pagination limit)
    # Otherwise use standard pagination of 10 items
    if len(queries) == 1 and queries[0].name == "*":
        page_size = len(results)  # Return all results for wildcard
    else:
        page_size = 10
...
def get_artifact(
    ...
    print(f"[GET_BY_ID] Request for type={artifact_type}, id={id}")
    username = _validate_token(x_authorization)
    if not username:
        raise HTTPException(status_code=403, detail="Authentication failed due to invalid or missing AuthenticationToken.")
    ...
    if not artifact:
        print(f"[GET_BY_ID] Artifact {id} NOT FOUND in DynamoDB")
        raise HTTPException(status_code=404, detail="Artifact does not exist.")
    
    print(f"[GET_BY_ID] Found artifact: name={artifact['name']}, stored_type={artifact['type']}, requested_type={artifact_type}")
    if artifact["type"] != artifact_type:
        print(f"[GET_BY_ID] TYPE MISMATCH: stored={artifact['type']}, requested={artifact_type}")
        raise HTTPException(status_code=404, detail="Artifact does not exist.")
...
def get_artifact_by_name(
    ...
    print(f"[GET_BY_NAME] Searching for name: '{name}'")
    
    results = []
    all_names = []
    for artifact_id, artifact in _list_artifacts():
        all_names.append(artifact["name"])
        if artifact["name"] == name:
            print(f"[GET_BY_NAME] MATCH found: {artifact_id} - {artifact['name']} ({artifact['type']})")
            results.append(ArtifactMetadata(
                name=artifact["name"],
                id=artifact_id,
                type=artifact["type"]
    ...
    if not results:
        print(f"[GET_BY_NAME] NO MATCH for '{name}'. Available names: {', '.join(sorted(all_names)[:20])}")
        raise HTTPException(status_code=404, detail="No such artifact.")
    
    print(f"[GET_BY_NAME] Returning {len(results)} result(s)")
    return results
...
def get_artifact_by_regex(
    ...
    print(f"[REGEX] Searching with regex: '{regex_query.regex}'")
    ...
    try:
        ...
        print(f"[REGEX] Invalid regex pattern: {e}")
        raise HTTPException(status_code=400, detail="Invalid regex pattern.")
    
    results = []
    match_count = 0
    for artifact_id, artifact in _list_artifacts():
        # Search both name and README content
        name_match = pattern.search(artifact["name"])
        readme_match = pattern.search(artifact.get("readme", ""))
        
        if name_match or readme_match:
            match_count += 1
            match_type = "name" if name_match else "readme"
            print(f"[REGEX] Match #{match_count} in {match_type}: {artifact['name']} ({artifact['type']})")
            results.append(ArtifactMetadata(
                name=artifact["name"],
                id=artifact_id,
                type=artifact["type"]
    ...
    if not results:
        print(f"[REGEX] NO MATCHES found for regex '{regex_query.regex}'")
        raise HTTPException(status_code=404, detail="No artifact found under this regex.")
    
    print(f"[REGEX] Returning {len(results)} result(s)")
    return results
