...
# Seed admin user at module load (Lambda cold start)
try:
    print(f"[INIT] Attempting to seed admin user: {_DEFAULT_ADMIN_USERNAME}")
    _create_user(_DEFAULT_ADMIN_USERNAME, _DEFAULT_ADMIN_PASSWORD, is_admin=True)
    print(f"[INIT] ✓ Admin user seeded/verified: {_DEFAULT_ADMIN_USERNAME}")
except Exception as e:
    print(f"[INIT] ⚠️  Admin user seeding failed: {type(e).__name__}: {e}")
...
def get_artifact_by_name(
    ...
    # FastAPI automatically URL-decodes path parameters, so 'name' is already decoded
    results = []
...
def authenticate(auth_request: AuthenticationRequest = Body(...)):
    ...
    # CRITICAL: Ensure default admin exists before ANY authentication attempt
    # This handles Lambda cold start race conditions
    if username == _DEFAULT_ADMIN_USERNAME:
        try:
            print(f"[AUTH] Ensuring default admin exists before authentication")
            _create_user(_DEFAULT_ADMIN_USERNAME, _DEFAULT_ADMIN_PASSWORD, is_admin=True)
            print(f"[AUTH] Default admin verified/created")
        except Exception as e:
            print(f"[AUTH] Warning: Could not ensure admin exists: {e}")
    ...
    print(f"[AUTH] User found: {username}, validating password")
    
    # Verify password
    pw_hash = _hash_password(password, user["salt"])
    if pw_hash != user["password_hash"]:
        print(f"[AUTH] Password hash mismatch for user: {username}")
        raise HTTPException(status_code=401, detail="The user or password is invalid.")
