...
class TestSizeMetric(unittest.TestCase):
    ...
    def test_compute_falls_back_to_repo_when_no_safetensors(
        ...
        self.assertEqual(
            set(scores.keys()),
            set(SizeMetric.device_capacity_bits.keys()),
        )
...
class TestRampUpTimeMetric(unittest.TestCase):
    ...
    def test_compute_handles_browser_failure(
        ...
        mock_client = mock_hf_client_cls.return_value
        mock_client.request.side_effect = RuntimeError("hf down")
        metric = RampUpTime()
        score = metric.compute(
            {"model_url": "https://huggingface.co/acme/model"}
        )
...
class TestLicenseMetric(unittest.TestCase):
    ...
    def test_top_level_license_detected(
        ...
        mock_client = mock_hf_client_cls.return_value
        mock_client.request.return_value = {"license": "apache-2.0"}
    ...
    @patch("src.Metrics.PurdueClient")
    @patch("src.Metrics.HFClient")
    def test_repo_license_text_hint_detects_permissiveness(
        self,
        mock_hf_client_cls: MagicMock,
        mock_grok_client_cls: MagicMock,
        mock_browse: MagicMock,
    ) -> None:
        mock_browse.return_value = [("LICENSE", 4096)]
        mock_client = mock_hf_client_cls.return_value
        ...
        metric = LicenseMetric()
        score = metric.compute(
            {"model_url": "https://huggingface.co/acme/gnu"}
        )
    ...
    def test_normalize_license_handles_suffixes(self) -> None:
        metric = LicenseMetric()

        self.assertEqual(
            metric._normalize_license_name("Apache-2.0-only"),
            "apache-2.0",
        )
        self.assertEqual(
            metric._normalize_license_name("Apache-2.0-or-later"),
            "apache-2.0",
        )

    def test_normalize_license_scans_lists(self) -> None:
        metric = LicenseMetric()

        value = metric._normalize_license_name(["unknown", "MIT license"])
        self.assertEqual(value, "mit")

    @patch("src.Metrics.browse_hf_repo")
    @patch("src.Metrics.PurdueClient")
    @patch("src.Metrics.HFClient")
    def test_repo_license_alias_detected(
        self,
        mock_hf_client_cls: MagicMock,
        mock_grok_client_cls: MagicMock,
        mock_browse: MagicMock,
    ) -> None:
        mock_browse.return_value = [("LICENSE.md", 2048)]
        mock_client = mock_hf_client_cls.return_value
        mock_client.request.side_effect = [
            {"cardData": {}},
            "This project is distributed under the CC-BY 4.0 terms.",
        ]

        metric = LicenseMetric()
        score = metric.compute({"model_url": "https://huggingface.co/acme/cc"})
...
""".strip()
    ...
    @patch(
        "src.Metrics.injectHFBrowser",
        return_value="Model overview without metrics",
    )
    @patch("src.Metrics.PurdueClient")
    @patch("src.Metrics.HFClient")
    def test_llm_failure_defaults_to_zero(
        ...
        _mock_hf_client_cls.return_value.request.side_effect = RuntimeError(
            "hf fail"
        )
    ...
    def test_heuristic_detects_claims_when_llm_unavailable(
        ...
        metric = PerformanceClaimsMetric()
        mock_grok_client_cls.return_value.llm.side_effect = RuntimeError(
            "llm down"
        )
        _mock_hf_client_cls.return_value.request.side_effect = RuntimeError(
            "hf fail"
    ...
    @patch("src.Metrics._call_llm_with_retry", return_value=1.0)
    @patch("src.Metrics.injectHFBrowser")
    @patch("src.Metrics.PurdueClient")
    @patch("src.Metrics.HFClient")
    def test_llm_path_returns_score(
        self,
        mock_hf_client_cls: MagicMock,
        mock_grok_client_cls: MagicMock,
        mock_inject: MagicMock,
        ...
        mock_client = mock_hf_client_cls.return_value
        mock_client.request.return_value = "Overview without metrics"
        mock_inject.return_value = "Overview without metrics"
        ...
        self.assertEqual(score, 1.0)
        mock_llm_retry.assert_called_once()
        mock_grok_client_cls.return_value.llm.assert_not_called()
...
class TestBusFactorMetric(unittest.TestCase):
    ...
    @patch.object(
        BusFactorMetric,
        "_calculate_effective_maintainers",
        return_value=3.0,
    )
    @patch.object(
        BusFactorMetric,
        "github_commit_counts",
